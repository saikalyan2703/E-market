...
public class ItemDB {
    ...
    public List<Item> getItems(String user_id) throws SQLException, ClassNotFoundException{
        Connection conn = CommonUtils.getConnection();                               
         String sql = "select item_id, name, price, description, item_type from items where user_id!=(?) and buyer=0";
         PreparedStatement ps=conn.prepareStatement(sql);     
    ...
    public List<Item> userItems(int user_id) throws ClassNotFoundException, SQLException{
        Connection conn = CommonUtils.getConnection();                               
         String sql = "select item_id, name, price, description, item_type from items where user_id=(?)";
         PreparedStatement ps=conn.prepareStatement(sql);     
         ps.setInt(1, user_id);
         ResultSet rs=ps.executeQuery();
         List<Item> itemList=new ArrayList<Item>();
     ...
     public List<Item> getAllItems() throws SQLException, ClassNotFoundException{
        Connection conn = CommonUtils.getConnection();                               
         String sql = "select item_id, name, price, description, item_type from items";
         PreparedStatement ps=conn.prepareStatement(sql);     
         ResultSet rs=ps.executeQuery();
         List<Item> itemList=new ArrayList<Item>();
         Item item = null;
